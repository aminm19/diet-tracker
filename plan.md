...
- Extract the repeated double-card shell markup (`DaySummary`, `EntryList`, `EntryCard`, `AddFoodModal`, and now `GoalsModal` — five instances) into a shared `Card`/`CardShell` component or an `index.css` `@utility`, rather than copy-pasted arbitrary-value Tailwind.
...
- Unit 7 — Health score engine + settings: **todo, next up.**
- Unit 8 — Deploy: todo.
