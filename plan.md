...
Follow-ups logged after Unit 5/6/7's design reviews; none block shipping.

**Done** (as part of the Unit 9/10 dedup pass, see Status below): the card-shell and modal-chrome extraction items, and the `GoalsModal`/`HealthScoreSettingsModal` narrow-viewport, seed-race, and double-submit items were superseded by or folded into that pass where applicable (see Status for specifics on what actually landed vs. what's still open).
...
- `DateNav` and `App.tsx`: swap the stray `hover:text-black/60` for the existing `text-muted`/`text-ink` tokens; consider styling the native `<input type="date">`'s focus-visible frame so it doesn't look like a stock browser control against the rest of the custom UI.
- `GoalsModal`: check the fixed `grid-cols-2` fields layout on a narrow (~320px) viewport — may need `grid-cols-1` below `sm:` if cramped.
- `GoalsModal`: fields are seeded from the `goals` prop only at mount; if opened before `App`'s initial `getGoals()` fetch resolves, it won't pick up the just-arrived values (saving still works correctly, just overwrites).
- Double-submit isn't synchronously guarded in `GoalsModal`/`AddFoodModal` — two rapid submits before React flushes `submitting=true` fire two concurrent save calls.
- `HealthScoreSettingsModal`: initial-focus timing still lands on Close instead of the master toggle in practice (the `useModal` extraction preserved this exactly rather than fixing it, per instruction — see Status).
- `HealthScoreBadge`: disappearing after the master toggle is turned off has no explicit screen-reader announcement.
